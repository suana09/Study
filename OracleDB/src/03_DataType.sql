...
--	ex. 123.45 --소수점이 1자리, 소수점 아래가 2자리 > 정수부분 3자리

--3. 가변형 문자 (최대 4000 Byte)
--	varchar2 (XX)
--	지정한 숫자를 저장할 수 있는 최대 글자수를 지정하고 할당되는 용량은 실제 글자수만큼 할당되어 저장됨
--	ex. varchar(10) 에 "abcd" 를 저장하면 4글자만큼의 용량만 소비됨

--4. 고정형 문자 (최대 2000 Byte)
--	char(XX)
--	char(10) 에 "abcd" 를 저장하면 " abcd      " 가 저장 (실제로 저장한 글자는 4글자뿐이어도 10글자 분량의 용량을 소비함)

--5. 고정형 유니코드 문자
--	char(XX) : 다국어를 입력하기 위한 고정형 자료형, 최대 2000Byte
-- 
--6. 가변형 유니코드 문자
...
--	SYSDATE : DATE 형식의 오늘날짜, 현재시간을 발생시키는 키워드
-- SYSTIMESTAMP : TIMESTAMP 형식의 오늘날짜 현재시간 (밀리초포함) 을 발생시키는 키워드
...
--	CLOB, NCLOB, BLOB 등이 있으나 요새는 대용량 파일은 별도의 서버에 저장하고 
--	경로와 이름을 데이터베이스로 관리하는 형식을 많이 사용하기 때문에 잘 사용하지 않음  
