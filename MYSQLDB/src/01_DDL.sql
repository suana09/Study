...
--auto increment : oracle의 sequence를 쓸 필요 없이 record를 하나 추가할 때마다 자동으로 +1된 숫자가 들어가는 키워드
...
--rentlist 테이블 생성 (without foreign keys)
CREATE TABLE `scott`.`rentlist` (
  `numseq` INT NOT NULL AUTO_INCREMENT,
...
--alter 로 foreign key 추가하기
ALTER TABLE `scott`.`rentlist` 
ADD INDEX `fk1_idx` (`bnum` ASC) VISIBLE, --oracle과의 다른 점 : index를 추가해줌
